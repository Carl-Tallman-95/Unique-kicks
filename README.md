...
- Navigation
    ...
    - The navigation bar is identical on each page to allow for easy navigation.
    - On any page you can click on the logo that is linked to getting to the home page.
...
- The Header Image And Text Box
    - The heading has an image as a background with text floating on its left side to contrast the navigation bar.
...
- How Does It Work Section
    ...
    - The steps give a clear guide to the user in order to understand the ease of use of the site.
    - The steps are easily readable with a pleasant design.

![This is an image](assets/images/readme_images/how_does_it_work.png)

- Check Out Our Gallery Section
    - This section will allow the user to get a glimpse into what the product is or could be.
...
- The Contact Us Page
    ...
    - The contact page encourages the users to detail the style they want for the service to work properly.
...
- I would have liked to add an upload button to the contact page so the user can upload their references more easily. It would also be useful if the user has a unique design that they've made.
...
**Solved bugs**
 1. - To make the header background image darker I used a div with a linear-gradient to overlay the image, but the overlay would block the nav-bar from being clickable.
    - I solved the issue by removing the div and then using the linear-gradient inside the background-image div property.
 ```
...
**Unfixed bugs**
- On the contact page, the description is responsive but not when you start to expand the text field area. I used a media query to change the size of the description but that stopped working when the text area was altered. 
```
...
>A special thanks to all of the resources that helped this project on the way to being what it is today!
